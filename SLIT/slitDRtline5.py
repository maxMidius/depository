...
def save_state(filename):
    ...
    state_data = {
        ...
        'deliverables': st.session_state.deliverables_data.to_dict('records')
    }
...
def restore_state(filename):
    ...
    try:
        ...
        # Restore releases
        st.session_state.releases = state_data['releases']

        # Restore deliverables
        deliverables_df = pd.DataFrame(state_data['deliverables'])
        # Convert string dates back to datetime
        deliverables_df['Completion Date'] = pd.to_datetime(deliverables_df['Completion Date'])
        st.session_state.deliverables_data = deliverables_df
        return True
    except Exception as e:
...
with st.sidebar:
    ...
    # Add Save/Restore section to sidebar
    st.subheader("Project State")

    # File name input
    project_file = st.sidebar.text_input("Project filename", "project_state.json")

    save_col1, save_col2, save_col3 = st.sidebar.columns(3)

    with save_col1:
        if st.button("Save"):
            if save_state(project_file):
                st.sidebar.success("Project saved!")

    with save_col2:
        if st.button("Save As"):
            new_filename = st.sidebar.text_input("Save as filename", "new_project.json")
            if st.sidebar.button("Confirm Save As"):
                if save_state(new_filename):
                    st.sidebar.success(f"Project saved as {new_filename}!")

    with save_col3:
        if st.button("Restore"):
            # Show available project files in current directory
            json_files = [f for f in os.listdir('.') if f.endswith('.json')]
            if json_files:
                file_to_restore = st.sidebar.selectbox("Select project to restore", json_files)
                if st.sidebar.button("Confirm Restore"):
                    if restore_state(file_to_restore):
                        st.sidebar.success("Project restored!")
            else:
                st.sidebar.warning("No project files found")
# Main content area
